...
/* ------------------------------------------- en-tete sur 1 ou 2 niveaux */

/**
 * Si la 1re ligne contient un colspan > 1 -> en-tete a 2 etages (defensif, pas
 * observe sur le site actuel mais possible) : ligne 0 = groupes, ligne 1 = libelles.
 * @return {{groups:string[], labels:string[], dataStart:number}}
 */
function analyzeHeader_(rows) {
  var r0 = getCellsEx_(rows[0]);
  var twoLevel = rows.length > 1 && r0.some(function (c) { return c.colspan > 1; });

  if (!twoLevel) {
    return {
      groups: r0.map(function () { return ''; }),
      labels: r0.map(function (c) { return c.text; }),
      dataStart: 1
    };
  }

  var queue = getCellsEx_(rows[1]);
  var groups = [], labels = [];
  r0.forEach(function (c) {
    if (c.rowspan >= 2) { groups.push(''); labels.push(c.text); }
    else {
      for (var k = 0; k < c.colspan; k++) {
        var sub = queue.shift();
        groups.push(c.text);
        labels.push(sub ? sub.text : (c.text + ' ' + (k + 1)));
      }
    }
  });
  return { groups: groups, labels: labels, dataStart: 2 };
}
...
function parseActesTable_(tableHtml, baseUrl, criteria, warnings) {
  var rows = getRows_(tableHtml);
  var h = analyzeHeader_(rows);
  var plan = analyzeActesColumns_(h.labels, h.groups);
...
function parseFormFields(html) {
  ...
  var reInput = /<input\b([^>]*)>/gi;
  while ((m = reInput.exec(html)) !== null) {
    var a = m[1];
    fields.push({
      tag: 'input',
      type: (a.match(/\btype\s*=\s*["']?([\w-]+)/i) || [, 'text'])[1].toLowerCase(),
      name: (a.match(/\bname\s*=\s*["']?([^"'\s>]+)/i) || [, ''])[1],
      value: decodeEntities_((a.match(/\bvalue\s*=\s*["']([^"']*)["']/i) || [, ''])[1]),
      checked: /\bchecked\b/i.test(a)
    });
  }

  var reSelect = /<select\b([^>]*)>([\s\S]*?)<\/select>/gi;
  while ((m = reSelect.exec(html)) !== null) {
    var opts = [], reOpt = /<option\b([^>]*)>([\s\S]*?)<\/option>/gi, o;
    while ((o = reOpt.exec(m[2])) !== null) {
      opts.push({
        value: decodeEntities_((o[1].match(/\bvalue\s*=\s*["']?([^"'\s>]*)/i) || [, ''])[1]),
        label: cleanCell_(o[2]),
        selected: /\bselected\b/i.test(o[1])
      });
    }
    fields.push({
      tag: 'select', type: 'select',
      name: (m[1].match(/\bname\s*=\s*["']?([^"'\s>]+)/i) || [, ''])[1],
      options: opts
    });
  }

  var form = html.match(/<form\b([^>]*)>/i);
  return {
    action: form ? (form[1].match(/\baction\s*=\s*["']([^"']*)["']/i) || [, ''])[1] : '',
    method: form ? ((form[1].match(/\bmethod\s*=\s*["']?(\w+)/i) || [, 'get'])[1]).toLowerCase() : 'get',
    fields: fields.filter(function (f) { return f.name; })
  };
}
