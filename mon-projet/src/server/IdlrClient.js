...
/**
 * Payload du POST : les champs CACHES du <form> charge sont repris tels
 * quels (aujourd'hui code + rech=2 — le site les EXIGE, sans eux il renvoie
 * une page vide ; ils ont deja change par le passe), puis les criteres
 * demandes sont poses par-dessus.
 */
function buildPayload_(formHtml, q) {
  var payload = {}, h;
  var reHidden = /<input\b[^>]*type\s*=\s*["']?hidden["']?[^>]*>/gi;
  while ((h = reHidden.exec(String(formHtml || ''))) !== null) {
    var nom = (h[0].match(/\bname\s*=\s*["']?([^"'\s>]+)/i) || [, ''])[1];
    if (nom) payload[nom] = decodeEntities_((h[0].match(/\bvalue\s*=\s*["']([^"']*)["']/i) || [, ''])[1]);
  }
...
function search(q) {
  ...
  // 1) GET du formulaire -> cookie + perimetre en session + champs caches
